...
class PacketHeader(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ("m_packetFormat",              DataTypes.UNSIGNED_INT16.value),    # 2026
        ("m_gameYear",                  DataTypes.UNSIGNED_INT8.value),     # Game year - last two digits e.g. 25
...
class PacketMotionData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        ("m_carMotionData", CarMotionData * 24),    # Data for all cars on track
    ]
...
class ActiveAeroZone(DataTypes.STRUCTURE.value):
    _pack_ = 1 # !!REQUIRED - is required or error occurs
    _fields_ = [
        ("m_zoneStart", DataTypes.FLOAT.value), # Fraction (0..1) of way through the lap the Active Aero zone starts
        ("m_zoneEnd",   DataTypes.FLOAT.value), # Fraction (0..1) of way through the lap the Active Aero zone ends
    ]

class DRSZone(DataTypes.STRUCTURE.value):
    _pack_ = 1 # !!REQUIRED - is required or error occurs
    _fields_ = [
        ("m_zoneStart", DataTypes.FLOAT.value), # Fraction (0..1) of way through the lap the DRS zone starts
        ("m_zoneEnd",   DataTypes.FLOAT.value), # Fraction (0..1) of way through the lap the DRS zone ends
    ]
...
class PacketSessionData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        # Aero and DRS zones
        ("m_activeAeroTrackStatus",         DataTypes.UNSIGNED_INT8.value), # 0 = Full, 1 = Partial
        ("m_numActiveAeroZonesFull",        DataTypes.UNSIGNED_INT8.value), # Number of Active Aero zones to follow
        ("m_activeAeroZonesFull",           ActiveAeroZone * 8),            # List of Active Aero zones -max 8
        ("m_numActiveAeroZonesPartial",     DataTypes.UNSIGNED_INT8.value), # Number of Active Aero zones to follow
        ("m_activeAeroZonesPartial",        ActiveAeroZone * 8),            # List of Active Aero zones -max 8
        ("m_numDRSZones",                   DataTypes.UNSIGNED_INT8.value), # Number of DRS zones to follow
        ("m_drsZones",                      DRSZone * 4),                   # List of DRS zones -max 4
        ("m_startReactionTime",             DataTypes.FLOAT.value),         # Driver start reaction time in seconds 0.0f if assisted starts
        ("m_antiLockBrakesAssist",          DataTypes.UNSIGNED_INT8.value), # 0 = Off, 1 = On
        ("m_tractionControlAssist",         DataTypes.UNSIGNED_INT8.value), # 0 = Off, 1 = Medium, 2 = Full
        ("m_dynamicRacingLineHiVis",        DataTypes.UNSIGNED_INT8.value), # 0 = Off, 1 = On
        ("m_dynamicRacingLineColourBlind",  DataTypes.UNSIGNED_INT8.value), # 0 = Off, 1 = Protanopia, 2 = Deuteranopia, 3 = Tritanopia
        ("m_recurringRewindPrompt",         DataTypes.UNSIGNED_INT8.value), # 0 = Off, 1 = On
    ]
...
class PacketLapData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ("m_header",                PacketHeader),                      # Header
        ("m_lapData",               LapData * 24),                      # Lap data for all cars on track
...
class Retirement(DataTypes.STRUCTURE.value):
    _fields_ = [
        ...
        ("reason",      DataTypes.UNSIGNED_INT8.value)      # Reason - 0 = invalid, 1 = retired, 2 = finished, 3 = terminal damage, 4 = inactive, 5 = not enough laps completed, 6 = black flagged, 7 = red flagged, 8 = mechanical failure, 9 = session skipped, 10 = session simulated
    ]

class DRSDisabled(DataTypes.STRUCTURE.value):
    _fields_ = [
        ("reason",  DataTypes.UNSIGNED_INT8.value)  # 0 = Wet track, 1 = Safety car deployed, 2 = Red flag, 3 = Min lap not reached
    ]
...
class StopGoPenaltyServed(DataTypes.STRUCTURE.value):
    _fields_ = [
        ...
        ("stopTime",  DataTypes.FLOAT.value)                # Time spent serving stop go in seconds
    ]
...
class Collision(DataTypes.STRUCTURE.value):
    _fields_ = [
        ...
        ("severity",    DataTypes.UNSIGNED_INT8.value), # Severity of the collision -0 = low, 1 = medium, 2 = high
    ]
...
### Participants Packet -- Every 5 seconds -- 1470 bytes

class LiveryColour(DataTypes.STRUCTURE.value):
    _pack_ = 1
    _fields_ = [
        ("red",     DataTypes.UNSIGNED_INT8.value),
        ("green",   DataTypes.UNSIGNED_INT8.value),
        ("blue",    DataTypes.UNSIGNED_INT8.value),
    ]
...
class ParticipantData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        ("m_name",              DataTypes.CHAR.value * 32),         # Name of participant in UTF-8 format – null terminated, Will be truncated with … (U+2026) if too long
        ...
        ("m_numColours",        DataTypes.UNSIGNED_INT8.value),     # Number of colours valid for this car
...
class CarStatusData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        ("m_actualTyreCompound",        DataTypes.UNSIGNED_INT8.value),     # F1 Modern - 16 = C5, 17 = C4, 18 = C3, 19 = C2, 20 = C1, 21 = C0, 22 = C6, 7 = inter, 8 = wet
        ...
        ("m_ersHarvestedLimitPerLap",   DataTypes.FLOAT.value),             # ERS energy harvestlimit for this lap
...
class FinalClassificationData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        ("m_resultStatus",      DataTypes.UNSIGNED_INT8.value),         # Result status - 0 = invalid, 1 = inactive, 2 = active, 3 = finished, 4 = didnotfinish, 5 = disqualified, 6 = not classified, 7 = retired
        ("m_resultReason",      DataTypes.UNSIGNED_INT8.value),         # Result reason - 0 = invalid, 1 = retired, 2 = finished, 3 = terminal damage, 4 = inactive, 5 = not enough laps completed, 6 = black flagged, 7 = red flagged, 8 = mechanical failure, 9 = session skipped, 10 = session simulated
...
class PacketFinalClassificationData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        ("m_classificationData",    FinalClassificationData * 24),
    ]
...
class PacketLobbyInfoData(DataTypes.STRUCTURE.value):
    _fields_ = [
        ...
        ("m_lobbyPlayers",  LobbyInfoData * 24),
    ]
...
class CarDamageData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        ("m_tyreBlisters",          DataTypes.UNSIGNED_INT8.value * 4),     # Tyre blisters value (percentage)
...
class PacketMotionExData(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ...
        ("m_chassisPitch",              DataTypes.FLOAT.value),         # Pitch angle of the chassis relative to the direction of motion – radians
        ("m_wheelCamber",               DataTypes.FLOAT.value * 4),     # Camber of each wheel in radians
        ("m_wheelCamberGain",           DataTypes.FLOAT.value * 4),     # Camber gain for each wheel in radians, difference between active camber and dynamic camber
    ]
...
class TimeTrialDataSet(DataTypes.STRUCTURE.value):
    ...
    _fields_ = [
        ("m_carIdx",                DataTypes.UNSIGNED_INT8.value),     # Index of the car this data relates to
        ("m_teamId",                DataTypes.UNSIGNED_INT16.value),     # Team id - see appendix
        ("m_lapTimeInMS",           DataTypes.UNSIGNED_INT32.value),    # Lap time in milliseconds
...
class PacketLapPositionsData(DataTypes.STRUCTURE.value):
    _pack_ = 1
    _fields_ = [
        ("m_header",                    PacketHeader),                              # Header
        ("m_numLaps",                   DataTypes.UNSIGNED_INT8.value),             # Number of laps in the data
        ("m_lapStart",                  DataTypes.UNSIGNED_INT8.value),             # Index of the lap where the data starts, 0 indexed
        ("m_positionForVehicleIdx",     DataTypes.UNSIGNED_INT8.value * 50 * 24),   # Array holding the position of the car in a given lap, 0 if no record
    ]
...
class CarTelemetry2Data(DataTypes.STRUCTURE.value):
    _pack_ = 1
    _fields_ = [
        ("m_activeAeroMode",                DataTypes.UNSIGNED_INT8.value),     # 0 = Corner mode, 1 = Straight mode
        ("m_activeAeroAvailable",           DataTypes.UNSIGNED_INT8.value),     # 0 = not available, 1 = available
        ("m_activeAeroActivationDistance",  DataTypes.UNSIGNED_INT16.value),    # 0 = Active aero not available, non-zero – Active aero will be available in [X] metres
        ("m_overtakeAvailable",             DataTypes.UNSIGNED_INT8.value),     # 0 = not available, 1 = available
        ("m_overtakeActive",                DataTypes.UNSIGNED_INT8.value),     # 0 = not active, 1 = active
        ("m_overtakeActivationDistance",    DataTypes.UNSIGNED_INT16.value),    # 0 = Overtake Mode not available, non-zero – Overtake Mode will be available in [X] metres
        ("m_2026Regulations",               DataTypes.UNSIGNED_INT8.value),     # 0 = vehicle conforms to pre-2026, 1 = 2026 regulations applicable
        ("m_drivingWrongWay",               DataTypes.UNSIGNED_INT8.value),     # Whether the car is driving the wrong way
    ]

class PacketCarTelemetry2Data(DataTypes.STRUCTURE.value):
    _pack_ = 1
    _fields_ = [
        ("m_header",            PacketHeader),              # Header
        ("m_carTelemetry2Data", CarTelemetry2Data * 24),
    ]
...
class MetaData:
    ...
    packetInfo: dict[int, tuple[type, ...]] = {
        ...
        16: (PacketCarTelemetry2Data,),         # Additional telemetry data for all cars
    }
