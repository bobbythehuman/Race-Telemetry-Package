...
class PacketAData(DataTypes.STRUCTURE):
    ...
    _fields_ = [
        ...
        ("UNKNOWNFLOAT1",           DataTypes.FLOAT * 8),     # Unknown float (from original source)
...
### Packet B -- 316 bytes -- Frequency: 60Hz (60 times a second) -- inherits from packet a

class PacketBData(PacketAData):
    _fields_ = [
        # For Packet B
        ("wheelRotation",   DataTypes.FLOAT),  # Calculates the wheel rotation in radians
        ("UNKNOWNFLOAT2",   DataTypes.FLOAT),  # Unknown float
...
### Packet ~ -- 344 bytes -- Frequency: 60Hz (60 times a second) -- inherits from packet b

class PacketTildaData(PacketBData):
    _fields_ = [
        # For Packet Tilda
        ...
        ("UNKNOWNFLOAT3",       DataTypes.FLOAT),             # Unknown float
    ]

### Packet C -- 368 bytes -- Frequency: 60Hz (60 times a second) -- inherits from packet tilda

class PacketCData(PacketTildaData):
    # _pack_ = 1
    _enums_: dict[type, tuple[str, ...]] = {
    ...
    _fields_ = [
        ...
        ("UNKNOWNFLOAT4",   DataTypes.FLOAT * 3),         # Unknown float
