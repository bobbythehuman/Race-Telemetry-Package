...
class ActiveAeroZone(DataTypes.STRUCTURE):
    _pack_ = 1 # !!REQUIRED - is required or error occurs
    _fields_ = [
        ("m_zoneStart", DataTypes.FLOAT), # Fraction (0..1) of way through the lap the Active Aero zone starts
        ("m_zoneEnd",   DataTypes.FLOAT), # Fraction (0..1) of way through the lap the Active Aero zone ends
    ]

class DRSZone(DataTypes.STRUCTURE):
    _pack_ = 1 # !!REQUIRED - is required or error occurs
    _fields_ = [
        ("m_zoneStart", DataTypes.FLOAT), # Fraction (0..1) of way through the lap the DRS zone starts
        ("m_zoneEnd",   DataTypes.FLOAT), # Fraction (0..1) of way through the lap the DRS zone ends
    ]
...
class PacketSessionData(DataTypes.STRUCTURE):
    ...
    _fields_ = [
        ...
        # Aero and DRS zones
        ("m_activeAeroTrackStatus",         DataTypes.UNSIGNED_INT8), # 0 = Full, 1 = Partial
        ("m_numActiveAeroZonesFull",        DataTypes.UNSIGNED_INT8), # Number of Active Aero zones to follow
        ("m_activeAeroZonesFull",           ActiveAeroZone * 8),            # List of Active Aero zones -max 8
        ("m_numActiveAeroZonesPartial",     DataTypes.UNSIGNED_INT8), # Number of Active Aero zones to follow
        ("m_activeAeroZonesPartial",        ActiveAeroZone * 8),            # List of Active Aero zones -max 8
        ("m_numDRSZones",                   DataTypes.UNSIGNED_INT8), # Number of DRS zones to follow
        ("m_drsZones",                      DRSZone * 4),                   # List of DRS zones -max 4
        ("m_startReactionTime",             DataTypes.FLOAT),         # Driver start reaction time in seconds 0.0f if assisted starts
        ("m_antiLockBrakesAssist",          DataTypes.UNSIGNED_INT8), # 0 = Off, 1 = On
        ("m_tractionControlAssist",         DataTypes.UNSIGNED_INT8), # 0 = Off, 1 = Medium, 2 = Full
        ("m_dynamicRacingLineHiVis",        DataTypes.UNSIGNED_INT8), # 0 = Off, 1 = On
        ("m_dynamicRacingLineColourBlind",  DataTypes.UNSIGNED_INT8), # 0 = Off, 1 = Protanopia, 2 = Deuteranopia, 3 = Tritanopia
        ("m_recurringRewindPrompt",         DataTypes.UNSIGNED_INT8), # 0 = Off, 1 = On
    ]
...
class Collision(DataTypes.STRUCTURE):
    _fields_ = [
        ...
        ("severity",    DataTypes.UNSIGNED_INT8), # Severity of the collision -0 = low, 1 = medium, 2 = high
    ]
...
class CarStatusData(DataTypes.STRUCTURE):
    ...
    _fields_ = [
        ...
        ("m_ersHarvestedLimitPerLap",   DataTypes.FLOAT),             # ERS energy harvestlimit for this lap
...
class PacketLapPositionsData(DataTypes.STRUCTURE):
    ...
    _fields_ = [
        ...
        ("m_positionForVehicleIdx",     DataTypes.UNSIGNED_INT8 * 50 * 24),   # Array holding the position of the car in a given lap, 0 if no record
    ]
...
class CarTelemetry2Data(DataTypes.STRUCTURE):
    _pack_ = 1
    _fields_ = [
        ("m_activeAeroMode",                DataTypes.UNSIGNED_INT8),     # 0 = Corner mode, 1 = Straight mode
        ("m_activeAeroAvailable",           DataTypes.UNSIGNED_INT8),     # 0 = not available, 1 = available
        ("m_activeAeroActivationDistance",  DataTypes.UNSIGNED_INT16),    # 0 = Active aero not available, non-zero – Active aero will be available in [X] metres
        ("m_overtakeAvailable",             DataTypes.UNSIGNED_INT8),     # 0 = not available, 1 = available
        ("m_overtakeActive",                DataTypes.UNSIGNED_INT8),     # 0 = not active, 1 = active
        ("m_overtakeActivationDistance",    DataTypes.UNSIGNED_INT16),    # 0 = Overtake Mode not available, non-zero – Overtake Mode will be available in [X] metres
        ("m_2026Regulations",               DataTypes.UNSIGNED_INT8),     # 0 = vehicle conforms to pre-2026, 1 = 2026 regulations applicable
        ("m_drivingWrongWay",               DataTypes.UNSIGNED_INT8),     # Whether the car is driving the wrong way
    ]

class PacketCarTelemetry2Data(DataTypes.STRUCTURE):
    _pack_ = 1
    _fields_ = [
        ("m_header",            PacketHeader),              # Header
