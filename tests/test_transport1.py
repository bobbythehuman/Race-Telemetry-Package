"""
Comprehensive pytest suite for RaceTelemetry.transport
=======================================================

Covers ``UDPTransport`` and ``SharedMemoryTransport`` in transport.py.
...
# from RaceTelemetry.transport import SharedMemoryTransport, UDPTransport
from ..src.RaceTelemetry.transport import (
...
def udp_transport(config, router, supervisor):
    ...
    # In real usage, get_udp_packets() sets FULLBUFFERSIZE before the loop
...
class TestUDPTransportInit:
    ...
    def test_initial_state(self, udp_transport):
        assert udp_transport.heartBeatDestination is None
        assert udp_transport._packet_counter == 0
        assert udp_transport._full_buffer_size == 0

    def test_heartbeat_interval_class_constant(self):
        assert UDPTransport.HEARTBEAT_INTERVAL == 5


# ---------------------------------------------------------------------------
# UDPTransport.call_handshake
# ---------------------------------------------------------------------------


class TestCallHandshake:
    def test_no_handshake_func_configured_is_a_noop(self, udp_transport):
        sock = MagicMock()
        udp_transport.config.handshake_func = None

        udp_transport.call_handshake(sock, "start")  # must not raise

        sock.assert_not_called()

    def test_start_mode_calls_first_handshake_function(self, udp_transport):
        sock = MagicMock()
        start_fn, stop_fn = MagicMock(), MagicMock()
        udp_transport.config.handshake_func = (start_fn, stop_fn)
        udp_transport.config.destination_ip = "10.0.0.5"
        udp_transport.config.handshake_port = 9999

        udp_transport.call_handshake(sock, "start")

        start_fn.assert_called_once_with(sock, ("10.0.0.5", 9999))
        stop_fn.assert_not_called()

    def test_stop_mode_calls_second_handshake_function(self, udp_transport):
        sock = MagicMock()
        start_fn, stop_fn = MagicMock(), MagicMock()
        udp_transport.config.handshake_func = (start_fn, stop_fn)
        udp_transport.config.destination_ip = "10.0.0.5"
        ...
        udp_transport.call_handshake(sock, "stop")

        stop_fn.assert_called_once_with(sock, ("10.0.0.5", 9999))
        start_fn.assert_not_called()

    def test_unknown_mode_calls_neither_function(self, udp_transport):
        sock = MagicMock()
        start_fn, stop_fn = MagicMock(), MagicMock()
        udp_transport.config.handshake_func = (start_fn, stop_fn)
        udp_transport.config.destination_ip = "10.0.0.5"

        udp_transport.call_handshake(sock, "pause")  # not "start" or "stop"
...
# ---------------------------------------------------------------------------
# UDPTransport.get_udp_packets
# ---------------------------------------------------------------------------


class TestGetUdpPackets:
    def test_raises_when_handshake_configured_without_destination_ip(self, udp_transport):
        udp_transport.config.handshake_func = (MagicMock(), MagicMock())
        udp_transport.config.destination_ip = None

        with pytest.raises(ValueError, match="Destination IP"):
            next(udp_transport.get_udp_packets())
    ...
    def test_happy_path_binds_handshakes_and_yields_expected_number_of_packets(self, mock_socket_cls, udp_transport, router, supervisor):
        ...
        with patch.object(udp_transport, "_process_loop", return_value=("pkt", 1, "hdr")) as mock_loop:
            results = list(udp_transport.get_udp_packets())
    ...
    @patch("RaceTelemetry.src.RaceTelemetry.transport.socket.socket")
    def test_bind_failure_triggers_supervisor_stop_and_skips_loop(self, mock_socket_cls, udp_transport, supervisor):
        mock_sock = MagicMock()
    ...
    @patch("RaceTelemetry.src.RaceTelemetry.transport.socket.socket")
    def test_socket_closed_even_if_never_active(self, mock_socket_cls, udp_transport, supervisor):
        mock_sock = MagicMock()
        mock_socket_cls.return_value = mock_sock
        supervisor._is_still_active.return_value = False  # loop body never runs

        list(udp_transport.get_udp_packets())

        mock_sock.close.assert_called_once()


# ---------------------------------------------------------------------------
# UDPTransport._process_loop
# ---------------------------------------------------------------------------
...
class TestProcessLoop:
    ...
    ever set inside ``get_udp_packets`` (never in ``__init__``). The
    ``udp_transport`` fixture sets it manually so these tests can call
    ``_process_loop`` in isolation, mirroring what ``get_udp_packets``
    would have done before entering its loop.
    """

    def test_raises_if_heartbeat_func_not_callable(self, udp_transport):
        udp_transport.config.heartbeat_func = "not-a-function"
        sock = MagicMock()

        with pytest.raises(ValueError, match="Heart Beat Function"):
            udp_transport._process_loop(sock)

    def test_raises_if_decryption_func_not_callable(self, udp_transport):
        udp_transport.config.decryption_func = "not-a-function"
        sock = MagicMock()
...
class TestProcessLoopHeartbeat:
    ...
    def test_heartbeat_on_timeout_resets_packet_counter(self, udp_transport):
        udp_transport._packet_counter = 4
        udp_transport.config.heartbeat_func = MagicMock()
        sock = MagicMock()
        sock.recvfrom.side_effect = TimeoutError
...
# ---------------------------------------------------------------------------
# SharedMemoryTransport.__init__
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SharedMemoryTransport.connect_map
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SharedMemoryTransport.setup_maps
# ---------------------------------------------------------------------------


class TestSetupMaps:
    def test_raises_if_packet_info_missing(self, sm_transport):
        sm_transport.config.packet_info = None

        with pytest.raises(ValueError, match="Packet Info"):
            sm_transport.setup_maps("AnyName")

    def test_raises_if_packet_info_empty_dict(self, sm_transport):
        sm_transport.config.packet_info = {}

        with pytest.raises(ValueError, match="Packet Info"):
            sm_transport.setup_maps("AnyName")

    def test_string_name_calls_connect_map_once(self, sm_transport):
        sm_transport.config.packet_info = {1: []}  # merely needs to be truthy

        with patch.object(sm_transport, "connect_map", return_value={"MAP": 100}) as mock_connect:
            result = sm_transport.setup_maps("SharedMapName")

        mock_connect.assert_called_once_with("SharedMapName")
        assert result == {"MAP": 100}

    def test_dict_name_connects_only_matching_struct_names(self, sm_transport):
        class PacketA:
            pass
        ...
        sm_transport.config.packet_info = {1: [PacketA, PacketB]}
        ...
        with patch.object(sm_transport, "connect_map", side_effect=fake_connect_map) as mock_connect:
            result = sm_transport.setup_maps(name_map)

        mock_connect.assert_called_once_with("SharedA", PacketA)
        assert result == {"mapA": 10}

    def test_invalid_name_type_raises_value_error(self, sm_transport):
        sm_transport.config.packet_info = {1: []}

        with pytest.raises(ValueError, match="string or a dict"):
            sm_transport.setup_maps(12345)  # not str or dict


# ---------------------------------------------------------------------------
# SharedMemoryTransport.get_shared_packets
# ---------------------------------------------------------------------------


class TestGetSharedPackets:
    def test_raises_if_shared_memory_names_not_set(self, sm_transport):
        sm_transport.config.all_shared_memory_names = None

        with pytest.raises(ValueError, match="Shared memory name is not set"):
            next(sm_transport.get_shared_packets())
    ...
    def test_yields_packet_for_nonzero_data_and_skips_all_zero_data(self, sm_transport, router, supervisor):
        ...
        with patch.object(sm_transport, "setup_maps", return_value=setup_result):
            results = list(sm_transport.get_shared_packets())
    ...
    def test_all_maps_seeked_to_start_before_each_read(self, sm_transport, supervisor):
        sm_transport.config.all_shared_memory_names = "SomeMap"
        supervisor._is_still_active.side_effect = [True, False]
        ...
        with patch.object(sm_transport, "setup_maps", return_value={live_map: 1}):
            list(sm_transport.get_shared_packets())

        live_map.seek.assert_called_once_with(0)

    def test_keyboard_interrupt_triggers_stop(self, sm_transport, supervisor):
        sm_transport.config.all_shared_memory_names = "SomeMap"
        supervisor._is_still_active.side_effect = [True, False]
    ...
    def test_os_error_triggers_stop(self, sm_transport, supervisor):
        sm_transport.config.all_shared_memory_names = "SomeMap"
        supervisor._is_still_active.side_effect = [True, False]
    ...
    def test_maps_closed_after_loop_ends(self, sm_transport, supervisor):
        sm_transport.config.all_shared_memory_names = "SomeMap"
        supervisor._is_still_active.return_value = False  # loop body never runs
